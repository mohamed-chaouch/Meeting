...
export const createUser = async (req: Request, res : Response) => {
    try {
        ...
        const token = jwt.sign(
            {_id : newUser._id, username: newUser.username, password: newUser.password, imageUrl: newUser.imageUrl},
            "Mohamed@123",
            // { expiresIn: "1h" }
        )
...
export const loginUser = async (req: Request, res: Response) => {
    try {
      ...
      // Generate JWT token
      const token = jwt.sign(
        { _id: user._id, username: user.username, email: user.email, imageUrl: user.imageUrl },
        'Mohamed@123'
        // { expiresIn: '1h' }
      );
